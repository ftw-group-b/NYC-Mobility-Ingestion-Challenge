...
`notebooks/09_quality_gate/01_end_to_end_quality_gate.sql` combines the critical Bronze, Silver, Gold, dashboard, and analytics checks into one PASS/FAIL result. The final `assert_true` statement stops the job if any critical check fails.
